...
| `retainMaxInitialMessages` | `HINDSIGHT_RETAIN_MAX_INITIAL_MESSAGES` | `300` | Caps the *first* retain of a session to the last N messages. Protects against a legacy project's giant transcript blowing the server's retain wall-clock limit on first contact. `0` disables. Delta retains are unaffected. |
